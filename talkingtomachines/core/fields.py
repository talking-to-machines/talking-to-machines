...
scope is keyed by a task name and a field name.

Getter/setter API::

    state = ExperimentState()
    state.set_player(player_id, task, "decision", 5)
    val = state.get_player(player_id, task, "decision")
...
class ExperimentState:
    ...
    Internal storage layout::
        ...
        _session[task][name]                       → value
        _agent[agent_id][task][name]               → value
        _group[group_id][task][name]               → value
        _player[player_id][task][name]             → value
    """
    ...
    def set_session(self, task: str, name: str, value: Any) -> None:
        """Store a value at session scope.

        Args:
            task: Task identifier (e.g. ``"pgg"``).
            name: Field name within the task.
            value: The value to store.
        """
        with self._session_lock:
            self._session.setdefault(task, {})[name] = value

    def get_session(self, task: str, name: str, default: Any = None) -> Any:
        ...
        """
        with self._session_lock:
            return self._session.get(task, {}).get(name, default)

    def get_session_task(self, task: str) -> dict[str, Any]:
        """Return a snapshot of all session-scoped fields for a task.
        ...
        Returns:
            ...
            given task. Returns an empty dict if no fields are stored.
        """
        with self._session_lock:
            return dict(self._session.get(task, {}))

    # ------------------------------------------------------------------
    # Agent scope
    # ------------------------------------------------------------------

    def set_agent(self, agent_id: str, task: str, name: str, value: Any) -> None:
        ...
        with self._agent_lock:
            self._agent.setdefault(agent_id, {}).setdefault(task, {})[name] = value

    def get_agent(
        self, agent_id: str, task: str, name: str, default: Any = None
    ) -> Any:
        ...
        """
        with self._agent_lock:
            return self._agent.get(agent_id, {}).get(task, {}).get(name, default)

    def get_agent_task(self, agent_id: str, task: str) -> dict[str, Any]:
        """Return a snapshot of all agent-scoped fields for a task.
        ...
        """
        with self._agent_lock:
            return dict(self._agent.get(agent_id, {}).get(task, {}))

    # ------------------------------------------------------------------
    # Group scope
    # ------------------------------------------------------------------

    def set_group(self, group_id: str, task: str, name: str, value: Any) -> None:
        ...
        with self._group_lock:
            self._group.setdefault(group_id, {}).setdefault(task, {})[name] = value

    def get_group(
        self, group_id: str, task: str, name: str, default: Any = None
    ) -> Any:
        ...
        """
        with self._group_lock:
            return self._group.get(group_id, {}).get(task, {}).get(name, default)

    def get_group_task(self, group_id: str, task: str) -> dict[str, Any]:
        """Return a snapshot of all group-scoped fields for a task.
        ...
        """
        with self._group_lock:
            return dict(self._group.get(group_id, {}).get(task, {}))
    ...
    def accumulate_group(
        self, group_id: str, task: str, name: str, player_id: str, value: Any
    ) -> None:
        ...
        """
        with self._group_lock:
            bucket = self._group.setdefault(group_id, {}).setdefault(task, {})
    ...
    def accumulate_session(
        self, task: str, name: str, player_id: str, value: Any
    ) -> None:
        ...
        """
        with self._session_lock:
            bucket = self._session.setdefault(task, {})
            if name not in bucket or not isinstance(bucket[name], dict):
                bucket[name] = {}
            bucket[name][player_id] = value
    ...
    def set_player(self, player_id: str, task: str, name: str, value: Any) -> None:
        ...
        with self._player_lock:
            self._player.setdefault(player_id, {}).setdefault(task, {})[name] = value

    def get_player(
        self, player_id: str, task: str, name: str, default: Any = None
    ) -> Any:
        ...
        """
        with self._player_lock:
            return self._player.get(player_id, {}).get(task, {}).get(name, default)

    def get_player_task(self, player_id: str, task: str) -> dict[str, Any]:
        """Return a snapshot of all player-scoped fields for a task.
        ...
        """
        with self._player_lock:
            return dict(self._player.get(player_id, {}).get(task, {}))
    ...
    def build_jinja_context(
        ...
        Args:
            ...
            task: Task identifier used to look up scoped fields.
            round_number: The current round number (1-based).
            constants: Optional nested dict of task-level constants,
                exposed in templates as ``C.<task>.<name>``.
        ...
        player_state = self.get_player_task(player.player_id, task)
        player_profile.update(player_state)

        # Agent-level fields
        agent_fields = self.get_agent_task(agent.agent_id, task)

        # Group-level fields
        group_fields = self.get_group_task(group.group_id, task)

        # Session-level fields
        session_fields = self.get_session_task(task)
        ...
        # Task-level constant access: C.<task>.<name>
        if constants:
...
class _DeepNamespace:
    """Two-level attribute-access namespace for constants (``C.<task>.<name>``).
    ...
    Args:
        data: A dict mapping task names to dicts of constant values.
    """
    ...
    def __init__(self, data: dict):
        ...
        Args:
            data: Mapping of task names to field-value dicts.
        """
        self._data = {task: _Namespace(vals) for task, vals in data.items()}

    def __getattr__(self, name: str) -> Any:
        """Resolve a task name to its ``_Namespace``.

        Args:
            name: Task name to look up.

        Returns:
            A ``_Namespace`` instance for the requested task.

        Raises:
            AttributeError: If no constants exist for the given task.
        """
        try:
            return self._data[name]
        except KeyError:
            raise AttributeError(f"No constant task '{name}'")
